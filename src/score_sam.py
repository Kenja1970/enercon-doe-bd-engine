...
def clean_notice_type(value: Any) -> str:
...
def calculate_days_until_deadline(value: Any) -> int | None:
    ...
    # so pandas can subtract cleanly.
...
def assign_drop_reason(
    ...
    if construction_hits:
        return f"Likely construction-only: {', '.join(construction_hits[:5])}"

    if naics and naics not in TARGET_NAICS and naics not in SECONDARY_NAICS:
        return f"Non-target NAICS: {naics}"
...
def assign_action_today(
    ...
    if "construction-only" in risk_text:
        return "Check for design component"
...
def score_row(row: pd.Series) -> pd.Series:
    ...
    if bad_hits:
        penalty = min(60, len(bad_hits) * 15)
        score -= penalty
    ...
    if construction_hits:
        score -= min(25, len(construction_hits) * 8)
...
def main() -> None:
    ...
    # Manual review fields may be populated later by merge_review.py.
    front_cols = [
        "review_priority",
        "action_today",
        "customer_market",
    ...
    scored.head(20).to_csv(TOP_20_CSV, index=False)

    review_today = scored[scored["review_priority"] == "1 - Review Today"]
    validate_owner = scored[scored["review_priority"] == "2 - Validate Owner"]
    monitor = scored[scored["review_priority"] == "3 - Monitor"]
    no_go = scored[scored["review_priority"].isin(["4 - Low Priority", "5 - Drop"])]
    doe_nnsa = scored[scored["customer_market"] == "DOE / NNSA"]
    usace_gsa = scored[
    ...
    print(f"No-Go / Low Priority: {len(no_go)}")
    print(f"DOE / NNSA: {len(doe_nnsa)}")
    print(f"USACE / GSA / Federal Buildings: {len(usace_gsa)}")
    print(f"CSV saved to: {OUTPUT_CSV.resolve()}")
    print(f"Top 20 saved to: {TOP_20_CSV.resolve()}")
